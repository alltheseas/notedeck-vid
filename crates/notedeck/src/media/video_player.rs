...
use super::frame_queue::WindowsDecodeThread;
...
#[cfg(all(target_os = "windows", feature = "windows-native-video"))]
use super::windows_audio::{AudioFormatInfo, WindowsAudioPlayback};
#[cfg(all(target_os = "windows", feature = "windows-native-video"))]
...
pub struct VideoPlayer {
    ...
    /// Returns (VideoMetadata, Option<AudioFormatInfo>) on success
    #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
    windows_init_rx: Option<
        crossbeam_channel::Receiver<Result<(VideoMetadata, Option<AudioFormatInfo>), VideoError>>,
    >,
    /// Timestamp when Windows factory init started (for watchdog timeout)
    #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
    windows_init_started: Option<std::time::Instant>,
    /// Windows-specific decode thread (handles both video and audio)
    #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
    windows_decode_thread: Option<WindowsDecodeThread>,
    /// Windows native audio playback manager
    #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
    windows_audio: Option<WindowsAudioPlayback>,
}
...
impl VideoPlayer {
        ...
        if self.windows_init_rx.is_some() {
            ...
            if let Some(start) = self.windows_init_started {
                if start.elapsed() > std::time::Duration::from_secs(INIT_TIMEOUT_SECS) {
                    ...
                    if let Some(ref thread) = self.windows_decode_thread {
                        thread.stop();
                    }
                    self.windows_decode_thread = None;
            ...
            match rx.try_recv() {
                Ok(Ok((metadata, audio_format, audio_queue, audio_clock))) => {
                    ...
                    // Initialize native Windows audio using the queue/clock from decode thread
                    if let (Some(format), Some(queue), Some(clock)) =
                        (audio_format, audio_queue, audio_clock)
                    {
                        match WindowsAudioPlayback::new_with_queue(format, queue, clock) {
                            Ok(audio_playback) => {
                                self.audio_handle.set_available(true);
                                tracing::info!(
                                    "Windows native audio initialized: {}Hz, {} channels",
                                    audio_playback.format().sample_rate,
                                    audio_playback.format().channels
                                );
                                self.windows_audio = Some(audio_playback);
                            }
                            Err(e) => {
                                tracing::warn!("Failed to initialize Windows audio: {}", e);
                                self.audio_handle.set_available(false);
                            }
                        }
                    } else {
                        tracing::info!("No audio stream in video");
                        self.audio_handle.set_available(false);
                    }
        ...
        match receiver.try_recv() {
            ...
            Ok(Err(e)) => {
                ...
                if matches!(&e, VideoError::DecoderInit(msg) if msg == "__WINDOWS_USE_FACTORY__") {
                    ...
                    // Uses WindowsDecodeThread which handles both video AND audio
                    let url = self.url.clone();
                    let frame_queue = Arc::clone(&self.frame_queue);

                    let (windows_decode_thread, init_rx) = WindowsDecodeThread::new_from_factory(
                        move || {
                            ...
                            tracing::info!(
                                "Creating Windows decoder on decode thread (with native audio)"
                            );
                    ...
                    self.windows_decode_thread = Some(windows_decode_thread);
                    ...
                    // Native audio will be started when init completes
                    self.init_thread = None;
                    self.init_receiver = None;
        ...
        #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
        {
            ...
            let (windows_decode_thread, init_rx) = WindowsDecodeThread::new_from_factory(
                move || {
                    ...
                    tracing::info!(
                        "Creating Windows decoder on decode thread (sync init, native audio)"
                    );
                    WindowsVideoDecoder::new(&url, debug)
                },
                frame_queue,
            ...
            match init_rx.recv_timeout(std::time::Duration::from_secs(30)) {
                Ok(Ok((metadata, audio_format, audio_queue, audio_clock))) => {
                    ...
                    // Initialize native Windows audio using the shared queue/clock
                    if let (Some(format), Some(queue), Some(clock)) =
                        (audio_format, audio_queue, audio_clock)
                    {
                        match WindowsAudioPlayback::new_with_queue(format, queue, clock) {
                            Ok(audio_playback) => {
                                self.audio_handle.set_available(true);
                                tracing::info!(
                                    "Windows native audio initialized (sync): {}Hz, {} channels",
                                    audio_playback.format().sample_rate,
                                    audio_playback.format().channels
                                );
                                self.windows_audio = Some(audio_playback);
    ...
    pub fn play(&mut self) {
        // Windows: Use WindowsDecodeThread
        #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
        if let Some(ref thread) = self.windows_decode_thread {
            thread.play();
            ...
            // Start Windows native audio playback
            if let Some(ref mut audio) = self.windows_audio {
                audio.play();
            }
            return;
        }
    ...
    pub fn pause(&mut self) {
        ...
        if let Some(ref thread) = self.windows_decode_thread {
            ...
            // Pause Windows native audio playback
            if let Some(ref mut audio) = self.windows_audio {
                audio.pause();
            }
            return;
    ...
    pub fn duration(&self) -> Option<Duration> {
        // Windows: Check WindowsDecodeThread first
        #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
        if let Some(ref thread) = self.windows_decode_thread {
        ...
        // Non-Windows: Check generic decode thread
        if let Some(ref thread) = self.decode_thread {
            if let Some(dur) = thread.duration() {
                return Some(dur);
            }
    ...
    pub fn buffering_percent(&self) -> i32 {
        ...
        #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
        if let Some(ref thread) = self.windows_decode_thread {
            return thread.buffering_percent();
        }
    ...
    pub fn set_volume(&mut self, volume: u32) {
        ...
        // On Windows, audio is controlled through native playback
        #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
        if let Some(ref audio) = self.windows_audio {
            audio.set_volume(volume as f32 / 100.0);
        }
    }
    ...
    pub fn toggle_mute(&mut self) {
        ...
        if let Some(ref audio) = self.windows_audio {
            let volume = if self.muted {
                ...
                self.audio_handle.volume() as f32 / 100.0
            };
    ...
    fn update_frame(&mut self) {
        // Get audio clock position for A/V sync (Windows native audio only)
        #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
        let audio_clock_pos = self
            .windows_audio
            .as_ref()
            .filter(|a| a.is_playing())
            .map(|a| a.clock_position());

        #[cfg(not(all(target_os = "windows", feature = "windows-native-video")))]
        let audio_clock_pos: Option<std::time::Duration> = None;

        // Get the next frame, using audio clock for sync when available
        if let Some(frame) = self
            .scheduler
            .get_next_frame_with_clock(&self.frame_queue, audio_clock_pos)
        {
...
impl Drop for VideoPlayer {
    fn drop(&mut self) {
        // Windows: Stop the Windows decode thread (handles both video and audio)
        #[cfg(all(target_os = "windows", feature = "windows-native-video"))]
        if let Some(ref thread) = self.windows_decode_thread {
            thread.stop();
        }
        // WindowsAudioPlayback is cleaned up automatically when dropped

        // Non-Windows: Stop the decode thread
        if let Some(ref thread) = self.decode_thread {
            thread.stop();
        }

        // Stop the audio thread (non-Windows platforms with FFmpeg)
        #[cfg(all(feature = "ffmpeg", not(target_os = "android")))]
        if let Some(ref audio_thread) = self.audio_thread {
            audio_thread.stop();
        }
